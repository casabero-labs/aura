...
describe('OE4 experiment aggregation — Task 9', () => {
  it('derives the complete 3 × 3 matrix with five runs per cell', () => {
    ...
    expect(result.matrix.cells.every((cell) => cell.runCount === 5)).toBe(true);
  });
  ...
  it('produces descriptive statistics and independent best dimensions', () => {
    ...
    expect(result.overall.diagnosticF1).toMatchObject({ count: 45, median: 0.6, min: 0.2, max: 1 });
    expect(result.overall.totalLatencyMs.count).toBe(45);
