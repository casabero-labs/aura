...
describe('formal diagnosis evaluator', () => {
  it('computes the frozen primary denominator and leaves script evaluation pending', () => {
    ...
    run.diagnosis!.parsedOutput = {
      ...
      issues: [{ issueId: 'hygiene-moji-nombre', evidenceRefs: [], hypothesis: 'mojibake', confidence: 0.8, requiresHumanReview: true, limits: [] }],
      diagnosisBlocks: [{ issueId: 'hygiene-moji-nombre', ruleId: 'rule:mojibake', columnId: 'col:nombre', scope: 'column', observation: 'mojibake', recommendation: 'review encoding' }], limitations: [],
      generatedAt: '2026-07-11T12:00:00.000Z',
    };
    const envelope = {
      columns: [{ columnId: 'col:nombre', name: 'nombre', position: 0, isAmbiguous: false, isDuplicate: false }],
      issues: [{ issueId: 'hygiene-moji-nombre', ruleId: 'rule:mojibake', columnId: 'col:nombre', scope: 'column' as const, evidenceRefs: [], actionability: 'auto_safe' as const, automaticAuthorization: { authorized: true, conditionsMet: [], reason: '' } }],
      datasetSummary: { rowCount: 15, colCount: 9, duplicateRows: 1, delimiter: ',', score: 0 },
      evidence: { samples: [], columnStats: {} },
      datasetFingerprint: { sha256: 'a'.repeat(64), rowCount: 15, colCount: 9, delimiter: ',', generatedAt: '2026-01-01T00:00:00Z' },
    ...
    expect(evaluation.diagnosis.primary).toEqual(expect.objectContaining({ tp: 1, fp: 0, fn: 14 }));
