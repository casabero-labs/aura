...
export const runAudit = (data: Record<string, any>[], fields: string[], delimiter: string): AuditReport => {
  ...
  if (duplicateCount > 0) {
    ...
    addDeduction(`Filas Duplicadas (${duplicateCount})`, points, IssueCategory.INTEGRITY, RULE_IDS.EXACT_DUPLICATES);
    addIssue({
      id: 'integrity-dupes',
  ...
  fields.forEach(col => {
    ...
    if (stats.uniqueCount === 1 && rowCount > 10) {
      addDeduction(`Columna Constante [${col}]`, 5, IssueCategory.INTEGRITY, RULE_IDS.CONSTANT_COLUMN);
      addIssue({
    ...
    if (col.length > 60 || REGEX_HEADER_QUESTION.test(col) || headerWordCount > 8) {
      addDeduction(`Cabecera no técnica [${col}]`, 2, IssueCategory.SEMANTIC, RULE_IDS.HEADER_VERBOSE);
      addIssue({
    ...
    if (ghostSpaceCount > 0) {
      addDeduction(`Espacios Fantasma en [${col}]`, 3, IssueCategory.HYGIENE, RULE_IDS.TRIM_WHITESPACE);
    ...
    if (mojibakeCount > 0) {
      addDeduction(`Encoding Corrupto en [${col}]`, 8, IssueCategory.HYGIENE, RULE_IDS.MOJIBAKE);
    ...
    if (!shouldSkipCapitalizationChaos(col, stats, values, rowCount)) {
      ...
      if (capitalizationVariants.affectedRows > 0) {
        addDeduction(`Caos de Mayúsculas en [${col}]`, 3, IssueCategory.HYGIENE, RULE_IDS.CAPITALIZATION_CHAOS);
        addIssue({
          id: `hygiene-case-${col}`,
          column: col,
    ...
    if (doubleSpaceCount > 0) {
      addDeduction(`Espacios Múltiples en [${col}]`, 2, IssueCategory.HYGIENE, RULE_IDS.DOUBLE_SPACES);
    ...
    if (symbolChaosCount > 0) {
      addDeduction(`Símbolos Sospechosos en [${col}]`, 5, IssueCategory.HYGIENE, RULE_IDS.SUSPICIOUS_SYMBOLS);
    ...
    if (urlPatternCount > 0) {
      addDeduction(`Formatos URL Inválidos en [${col}]`, 5, IssueCategory.LOGIC, RULE_IDS.MALFORMED_URLS);
    ...
    if (burnedRangeCount > rowCount * 0.6 && stats.uniqueCount <= 20) {
      addDeduction(`Rangos quemados [${col}]`, 4, IssueCategory.SEMANTIC, RULE_IDS.BURNED_DEMOGRAPHIC_RANGES);
      addIssue({
    ...
    if (outlierCount > 0) {
      addDeduction(`Outliers en [${col}]`, 5, IssueCategory.LOGIC, RULE_IDS.EXTREME_OUTLIERS);
    ...
    if (outlierCountTukey > 0) {
      addDeduction(`Outliers Leves en [${col}]`, 2, IssueCategory.LOGIC, RULE_IDS.MILD_OUTLIERS);
    ...
    if (invalidEmailCount > 0) {
      addDeduction(`Correos Inválidos en [${col}]`, 10, IssueCategory.LOGIC, RULE_IDS.INVALID_EMAIL);
    ...
    // Group 5 Checks
    if (piiCount > 0) {
      addDeduction(`Hallazgo PII en [${col}]`, 20, IssueCategory.SEMANTIC, RULE_IDS.PII_DETECTED);
  ...
  datePairs.forEach(pair => {
    ...
    if (inconsistentCount > 0) {
      addDeduction(`Incoherencia Temporal [${pair.start} vs ${pair.end}]`, 10, IssueCategory.LOGIC, RULE_IDS.TEMPORAL_INCONSISTENCY);
      addIssue({
  ...
  fields.forEach(col => {
    ...
    if (leakage.count >= 3 && leakagePct >= 1) {
      ...
      addDeduction(`Contaminación Semántica de ID [${col}]`, 5, IssueCategory.SEMANTIC, RULE_IDS.ID_SEMANTIC_CONTAMINATION);
      addIssue({
