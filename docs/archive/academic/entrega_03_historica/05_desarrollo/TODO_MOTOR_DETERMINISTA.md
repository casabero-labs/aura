...
`ScoreDeduction` ahora expone `severity`, `weight` y `ruleId` para trazabilidad. Calibrado para mantener el score de Titanic en `[60, 90]`. Documentado en `docs/tercera_entrega_aura/04_resultados/catalogo_reglas_motor_determinista.md` (sección "Scoring Compuesto Ponderado"). Cubierto por `src/__tests__/compositeScoring.test.ts` (3 tests verticales).
