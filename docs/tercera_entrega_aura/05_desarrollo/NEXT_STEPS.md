...
Clasificar corridas de evidencia como attempted_failed, preliminary_valid o formal_valid. Continuar fortaleciendo AURA como producto técnico controlado. La cuarta entrega NO debe prepararse todavía. Solo se inició cuando el usuario lo solicite explícitamente.
...
| L2 | Controlled Dataset Protocol | Definir dataset controlado ampliado, sin PII, con ground truth documentado | dataset protocol, schema, ground truth, claims | Completado |
| L3 | Controlled Pilot Run | Ejecutar flujo completo sobre dataset controlado ampliado | audit JSON, issues CSV, script, notebook, improvement run JSON | Completado |
